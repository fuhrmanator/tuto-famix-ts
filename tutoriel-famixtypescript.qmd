...
description: | 
  Ce tutoriel vous expliquera une démarche pour importer dans Moose un modèle d'un projet TypeScript et d'en faire une analyse. (actualisé pour Moose 12)
lang: fr-FR
author: 
  - "Christopher Fuhrman"
...
1.  Créez une image Moose avec Pharo Launcher. Pour la version actuelle de `ts2famix`, il faut utiliser le *template* **Moose Suite 12 (stable)**.
2.  Chargez le métamodèle `FamixTypeScript` dans Moose.

## Créer une image Moose dans Pharo

-   Installez [Pharo Launcher](https://pharo.org/web/download) et lancez-le.
-   Cliquez sur le bouton **New**.
-   Sélectionnez **Official distributions** et **Moose Suite 12 (stable)**.
-   Donnez un nom à l'image, par exemple, **Moose Suite 12 - TypeScript**.
-   Cliquez sur **Create image** pour créer l'image. Cela peut prendre quelques minutes.
...
-   Sélectionnez l'image Moose (créée à l'étape précédente) dans Pharo Launcher et démarrez-la (bouton **Launch**).
-   Dans Pharo, tapez <kbd>CTRL</kbd>-<kbd>O</kbd> <kbd>CTRL</kbd>-<kbd>W</kbd> pour ouvrir une fenêtre **Playground**.
...
-   Sélectionner tout le texte et appuyez sur <kbd>CTRL</kbd>-<kbd>G</kbd> (*Do it and go*) pour exécuter la commande. Le métamodèle devrait être chargé à la fin.
...
Ce code lit le fichier JSON (qui est dans le répertoire de travail actuel de l'image Pharo) et importe le modèle dans Moose.
Il faut avoir déplacé le fichier JSON au bon répertoire (faites un clic droit sur l'image et sélectionnez **Show in folder** pour ouvrir le dossier dans votre système d'exploitation).
